---
name: seo
description: "When the user wants to: xây dựng chiến lược SEO để tăng thứ hạng từ khóa, Organic Traffic và độ uy tín (Authority). Kích hoạt khi nhắc đến 'lên top Google', 'nghiên cứu từ khóa', 'audit website', 'backlink', 'on-page' hoặc 'traffic tự nhiên'."
version: 2.1.0
---

# SEO Strategy (Chiến lược Tối ưu Tìm kiếm)
...
1. **Nghiên cứu Từ khóa & Ý định Tìm kiếm (Keyword & Intent Research)**:
    - Phân tích Search Intent: Informational (Tìm thông tin), Navigational (Tìm trang cụ thể), Commercial (Tìm hiểu để mua), Transactional (Mua ngay).
    - Khai thác "Long-tail Keywords": Những từ khóa dài, cụ thể, ít cạnh tranh nhưng tỷ lệ chuyển đổi cao.
    - Nhóm từ khóa (Keyword Clustering) để xây dựng hệ thống Pillar-Cluster.

2. **Tối ưu On-page & Nội dung (On-page Excellence)**:
    - Chế tác Title Tag & Meta Description để tối ưu CTR.
    - Cấu trúc Heading (H1-H4) logic, dễ đọc.
    - Tối ưu hóa thực thể (Entity SEO) và Semantic Keywords (từ khóa đồng nghĩa/liên quan).

3. **Technical SEO & Core Web Vitals (Nền tảng Kỹ thuật)**:
    - Tối ưu tốc độ tải trang, tính thân thiện di động (Mobile-first).
    - Cài đặt Schema Markup (Dữ liệu cấu trúc) để xuất hiện dưới dạng Rich Snippets.
    - Đảm bảo Crawlability (Khả năng cào dữ liệu) và Indexability (Khả năng lập chỉ mục).
...
2. **Keyword Opportunity Map**: Bảng danh sách từ khóa kèm Volume, Difficulty và Intent.
...
## Edge cases
- **Thị trường cạnh tranh cực cao**: Chuyển sang chiến lược "Long-tail SEO" hoặc tập trung vào "Local SEO" (SEO địa phương) để chiếm lĩnh các khu vực ngách trước.
- **Website bị mất thứ hạng đột ngột**: Thực hiện "Penalty Audit" để kiểm tra xem có bị dính thuật toán mới của Google hay lỗi kỹ thuật nghiêm trọng không.
